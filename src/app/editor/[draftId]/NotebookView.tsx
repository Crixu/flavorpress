 ...
 * surface (WordPress, an editor, paper); we show notes - angles,
...
export function NotebookView({
  ...
  return (
    <article className="fp-notebook">
      <header className="fp-notebook-h">
        ...
        <h1 className="fp-notebook-title">{topic}</h1>
        <p className="fp-notebook-lede">
        ...
        <div className="fp-notebook-header-actions">
      ...
      <section className="fp-notebook-section">
        <h2 className="fp-notebook-section-h">
          <span className="fp-notebook-section-dot" style={{ background: "#9C4A22" }} />
          Angles
          <span className="fp-notebook-section-count">{notes.ideas.length}</span>
        </h2>
        <p className="fp-notebook-section-desc">Angles you might take. Pick one to write from.</p>
        {notes.ideas.length > 0 ? (
          <ul className="fp-notebook-angles">
            {notes.ideas.map((idea, i) => (
              <IdeasItem key={i} idea={idea} />
            ))}
          </ul>
        ) : (
          <p className="fp-notebook-empty">No angles yet.</p>
        )}
        <div className="fp-notebook-section-actions">
      ...
      <section className="fp-notebook-section">
        <h2 className="fp-notebook-section-h">
          <span className="fp-notebook-section-dot" style={{ background: "var(--fg-muted)" }} />
          Quotes
        ...
        {notes.quotes.length > 0 ? (
          ...
          <p className="fp-notebook-empty">No quotes yet.</p>
        )}
        <div className="fp-notebook-section-actions">
      ...
      <section className="fp-notebook-section">
        <h2 className="fp-notebook-section-h">
          <span
            className="fp-notebook-section-dot"
            style={{ background: "var(--border-strong)" }}
          />
          Leads
          <span className="fp-notebook-section-count">{notes.facts.length}</span>
        </h2>
        <p className="fp-notebook-section-desc">
          Paraphrased; verify against the source before publishing.
        </p>
        {notes.facts.length > 0 ? (
          <ul className="fp-notebook-facts">
            {notes.facts.map((f, i) => (
              <FactItem key={i} fact={f} />
            ))}
          </ul>
        ) : (
          <p className="fp-notebook-empty">No leads yet.</p>
        )}
      </section>

      <section className="fp-notebook-section">
        <h2 className="fp-notebook-section-h">
          <span className="fp-notebook-section-dot" style={{ background: "var(--fg-subtle)" }} />
          Sources
          <span className="fp-notebook-section-count">{sources.length}</span>
        </h2>
        <p className="fp-notebook-section-desc">
          Where the notes came from. Open one to read the original.
        </p>
        {sources.length > 0 ? (
          <ul className="fp-notebook-sources">
            {sources.map((row) => (
        ...
        <div className="fp-notebook-section-actions">
          <div className="fp-notebook-add-source">
            <p className="fp-notebook-add-source-hint">
...
function IdeasItem({ idea }: { idea: NoteIdea }) {
  return (
...
function QuoteItem({ quote }: { quote: NoteQuote }) {
  return (
    <li className="fp-notebook-quote">
      <blockquote>
...
function FactItem({ fact }: { fact: NoteFact }) {
  return (
    <li className="fp-notebook-fact">
      <p>{fact.text}</p>
      <a
        href={fact.sourceUrl}
        target="_blank"
...
function SourceItem({ row }: { row: SourceRow }) {
  return (
    <li className="fp-notebook-source">
      <a
        href={row.source_url}
        target="_blank"
        rel="noreferrer"
        className="fp-notebook-source-name hover:underline"
      >
        {row.display_name ?? hostFromUrl(row.source_url)}
      </a>
      <div className="fp-notebook-source-meta">{relativeTime(row.published_at)}</div>
      <div className="fp-notebook-source-title">{row.title}</div>
    </li>
  );
}

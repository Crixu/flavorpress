...
export const GLOSSARY: Record<string, GlossaryEntry> = {
  trust: {
    ...
    term: "Source trust",
    short:
      "Per-source confidence score that contributes 15% to a cluster's ranker composite. 0.5 default; learns from your behavior over time.",
    body: [
  ...
  ranker: {
    ...
    short:
      "3-signal heuristic that scores every cluster for every user. The Today screen shows the top 3 by composite.",
    body: [
  ...
  "archive-overlap": {
    ...
    short:
      "How much a cluster overlaps with topics you've already written about. Heaviest signal at 55% of the composite.",
    body: [
  ...
  "beat-match": {
    ...
    term: "Beat match",
    short:
      "Fraction of a cluster's entities that show up in your top-50 archive entities. 30% of the composite.",
    body: [
  ...
  cluster: {
    ...
    term: "Cluster",
    short:
      "A group of items from different sources that the cluster engine has identified as covering the same story.",
    body: [
  ...
  "cluster-fire": {
    ...
    term: "Cluster fires",
    short:
      "The moment a forming cluster crosses the 3-source / 2-domain / 72-hour threshold and starts being scored for users.",
    body: [
  ...
  "voice-match": {
    ...
    short:
      "0-100 score for how stylistically close a generated draft is to your past writing. Computed via Burrows' Delta on function-word distribution.",
    body: [
    ...
    formula: "voice_match = max(0, round((1 - burrows_delta(generated, profile)) * 100))",
  ...
  "burrows-delta": {
    ...
    term: "Burrows' Delta",
    short:
      "A stylometric distance measure based on function-word distributions. Used to detect when a draft sounds like the writer's archive.",
    body: [
  ...
  "function-words": {
    ...
    term: "Function words",
    short:
      "Common, low-content words (the, of, and, in...) used in stylometric analysis to fingerprint a writer.",
    body: [
  ...
  "voice-profile": {
    ...
    term: "Voice profile",
    short:
      "Per-outlet stylometric fingerprint extracted from your last 50 WordPress posts. Built once, refreshes weekly.",
    body: [
  ...
  "signature-terms": {
    ...
    short:
      "Words you use 5x more than the baseline blog corpus. Define your beat as a vocabulary fingerprint.",
    body: [
  ...
  "banned-terms": {
    ...
    term: "Banned terms",
    short:
      "Words the draft generator must avoid. Default list includes typical AI-slop tells; user-extensible.",
    body: [
  ...
  "decay-weighting": {
    ...
    short:
      "Per-post weight that decays with age, so recent posts shape the voice profile more than old ones.",
    body: [
    ...
    formula: "weight(age_days) = age <= 30 ? 1.0 : age <= 90 ? 0.7 : age <= 365 ? 0.4 : 0.1",
  ...
  outlet: {
    ...
    term: "Outlet",
    short:
      "A connected WordPress site you publish to. One author can have many outlets, each with its own voice profile.",
    body: [
  ...
  "application-password": {
    ...
    short:
      "WordPress's built-in token-style auth (since 5.6). FlavorPress uses it for REST API access.",
    body: [
  ...
  preflight: {
    ...
    short:
      "Pre-flight validation that runs before the WordPress authorize redirect to catch failure modes early.",
    body: [
  ...
  "fact-check": {
    ...
    short:
      "Per-claim binary check against the cluster source bundle. Flagged claims surface in the Decision Strip.",
    body: [
  ...
  originality: {
    ...
    short:
      "Character n-gram overlap against the cluster sources. 0-100. Below 90 the draft is considered too close to source text.",
    body: [
  ...
  "lane-fit": {
    ...
    term: "Lane fit",
    short:
      "Per-user score that gates whether a fired cluster is worth pre-rendering and (in v1.1) pushing.",
    body: [
  ...
  "ranker-correction": {
    ...
    short:
      "Per-cluster-pattern down-weight you apply when a cluster is mis-ranked. Inherited by future similar clusters.",
    body: [
  ...
  "voice-fit": {
    ...
    term: "Voice fit (v1.1)",
    short:
      "Predicted voice-match score for a cluster before drafting. Deferred to v1.1 in the ranker.",
    body: [
  ...
  "gap-fill": {
    ...
    term: "Gap fill (v1.1)",
    short:
      "Inverse of how many of your sources have already covered a cluster. Boosts under-covered angles. Deferred to v1.1.",
    body: [
  ...
  capability: {
    ...
    term: "Capability",
    short:
      "A typed unit of work (cluster engine, voice draft generator, fact-check, etc). Plugs in via manifest.",
    body: [
  ...
  trace: {
    ...
    term: "Trace ID",
    short:
      "Per-pipeline ID that ties together cluster, sources, ranker, voice-match, prompt, and edit history for one draft.",
    body: [
  ...
  "wp-callback": {
    ...
    short:
      "/api/wp/callback — receives credentials from WordPress after the user clicks Approve on /wp-admin/authorize-application.php.",
    body: [
